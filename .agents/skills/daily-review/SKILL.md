...
### Step 2.5: 买入机会复盘 (Buy Opportunities Analysis)

`machine.json` 中的 `buy_opportunities` 提供当日最佳买入机会的结构化分析，分为三个优先级：

| 优先级 | 类别 | 字段路径 | 说明 |
|--------|------|---------|------|
| 1 | 连板涨停股 | `buy_opportunities.consecutive_board` | >=2板的涨停股，按板数降序 |
| 2 | 首板涨停股 | `buy_opportunities.first_board` | 首次涨停股，按涨幅降序 |
| 3 | 日内低吸反转 | `buy_opportunities.intraday_surge` | 非涨停但从低点到收盘涨>=10%的股 |

每只股票的核心分析维度：

1. **涨停类型** (`limit_up_type`): `一字板` / `T字板` / `换手板` / `打开回封`
2. **入场分析** (`entry_analysis`): `best_entry_price`/`best_entry_time` 理论最佳买点，`entry_difficulty` 难度（impossible/hard/moderate/easy）
3. **收益指标** (`return_metrics`): `low_to_close_pct` 低点到收盘收益，`open_to_close_pct` 开盘到收盘收益
4. **封板信息** (`seal_info`): `first_seal_time`/`last_seal_time`/`seal_duration_minutes`/`board_breaks_count`
5. **量价分析** (`volume_analysis`): `amount_yi` 成交额，`volume_at_entry_increasing` 入场点放量

AI Agent 在复盘时应重点关注：
- **可实操的机会**（`entry_difficulty` 为 easy 或 moderate）详细展开入场逻辑
- **不可实操但强**（一字板）作为市场强度信号纳入情绪分析
- 连板股的 `limit_up_type` 换手板/打开回封是实际能参与的机会
- `seal_info.seal_duration_minutes` 封板时长越长封板质量越高
...
### Step 4.5: 机会筛选（Opportunity Hunter）

> 引用框架：`.agents/skills/opportunity-hunter/SKILL.md`
> 模式库：`.agents/skills/opportunity-hunter/PATTERNS.md`

在策略匹配之前，先使用机会筛选框架过滤出真正值得参与的机会：

1. **提取全部潜在机会**：从 machine.json 的连板梯队、首板股、日内低吸反转、策略候选、炸板股中收集
2. **逐一归类评估**：每个机会必须归入两大核心模式之一：
   - **核心龙头**：最主流、最核心、最有人气、涨幅最高的个股（市场地位+资金认可+趋势质量+板块纵深+可操作性）
   - **结构性机会**：预期差驱动、时效性强、风险收益比好的套利机会（预期差+确认信号+催化剂+风险收益比+时效性）
3. **输出筛选结果**：不归入这两类的一律排除，避免精力分散

具体评估维度和子模式定义详见机会筛选框架 SKILL。

### Step 5: 策略匹配与触发选股指引 (Strategy Matching)

阅读**策略操作手册**：`/home/fy/myown/livetrade/.agents/skills/daily-review/STRATEGIES.md`

调用您的分析智能，将今日的**当前市场环境状态**（结合 Step 1~4.5 得出的指数状况、高度压制分布、情绪阶段周期、已筛选的机会列表）与手册内的 8 大策略要求进行打分匹配：
...
基于 `machine.json` 中 `buy_opportunities` 数据，分析当日可实操的买入机会。

#### 连板股买入机会

| 个股 | 高度 | 封板类型 | 入场价 | 入场时间 | 难度 | 低→收% | 封板时长 | 开板次数 |
|------|------|---------|--------|---------|------|--------|---------|---------|
| ... | N板 | 换手板/一字板/T字板 | ... | HH:MM | easy/moderate/hard/impossible | ... | N分钟 | N次 |

重点分析可实操的换手板/打开回封机会，一字板仅作为情绪强度信号。

#### 首板股买入机会

| 个股 | 涨幅 | 封板类型 | 入场价 | 入场时间 | 难度 | 低→收% | 成交额 |
|------|------|---------|--------|---------|------|--------|--------|
| ... | ... | ... | ... | HH:MM | ... | ... | N亿 |

#### 日内低吸反转股（低点到收盘涨>=10%）

| 个股 | 涨幅 | 入场价 | 入场时间 | 难度 | 低→收% | 入场放量 |
|------|------|--------|---------|------|--------|---------|
| ... | ... | ... | HH:MM | ... | ... | 是/否 |

对 `entry_difficulty` 为 easy 或 moderate 的机会做详细点评，分析入场信号和逻辑。
