...
当前项目使用的竞价快照字段，理解上要特别注意下面几点：

- `buy` / `sell`：
  竞价阶段这里不是传统连续竞价里的“买一价 / 卖一价”语义，而是当前撮合价。很多时候两者相等。
- `bid1_volume` / `ask1_volume`：
  在竞价阶段两者通常相等，表示当前撮合量，不应该被当成连续竞价里的买一量、卖一量去理解。
- `bid2_volume`：
  当撮合价已经在涨停价时，`bid2_volume` 可以近似理解为涨停价上额外未撮合的买单量，也就是“显性封单”的一部分。
- `ask2_volume`：
  需要重点观察。它突然从很大变得很小，往往代表上方卖单被快速承接，不能只盯 `bid2_volume`。
- 因此竞价强弱不能机械地只看 `bid2_volume`，至少还要结合：
  - 是否重新站上涨停价
  - `ask2_volume` 是否明显收缩
  - 更深档位卖盘是否被快速吃掉
  - 最后一刻是否重新回封
...
- 竞价阶段不要把 `bid1_volume > ask1_volume` 当成“买盘强于卖盘”的依据。
- `auction_limit_chase` 这类策略后续应以“回到涨停价 + 卖盘显著收缩 + 显性封单恢复”来识别，而不是只看 `bid2_volume` 绝对值。
...
[竞价强度异动] sz300750 宁德时代 | 昨日涨停今日高开3.5%, 竞价量能放大
```
